...
def spatial_plot(shp_file, values, title):
    ...
    cutoff_high = 720  # e.g., shp[col].quantile(0.95)
    cutoff_low = 500   # e.g., shp[col].quantile(0.05)
    ...
    # Convert SiteID columns to same data type before merging
    shp_filtered_soybean['SiteID'] = shp_filtered_soybean['SiteID'].astype(int)
    values['SiteID'] = values['SiteID'].astype(int)
