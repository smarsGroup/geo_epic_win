...
def test_real_epic_runs_directly(workspace):
    ...
    folder, then execute EPIC1102.exe by hand and show exit code and output."""
    ...
    try:
        ...
        exes = glob.glob(os.path.join(run_dir, 'EPIC1102_*.exe'))
        assert exes, os.listdir(run_dir)
        r = subprocess.run([exes[0]], cwd=run_dir, input=b'\r\n' * 20, capture_output=True, timeout=120)
        ...
        print(f"\nreturncode={r.returncode} (hex {r.returncode & 0xFFFFFFFF:#x})\nproduced={produced}\n"
              f"run dir files={sorted(os.listdir(run_dir))}\n--- output ---\n{out[-3000:]}\n{_log_tails(ws)}")
        assert r.returncode == 0 and produced.get('umstead.ACY', 0) > 0
    finally:
        m.close()
